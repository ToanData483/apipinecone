#!/usr/bin/env python3
"""
Quick fix for SSL errors and filename conflicts
"""

def fix_sheets_reader():
    """Add retry logic to sheets reader"""
    
    sheets_reader_content = '''#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
'''
    ...
    with open('src/sheets_reader.py', 'w', encoding='utf-8') as f:
        f.write(sheets_reader_content)
    
    print("✅ Fixed: src/sheets_reader.py (with retry logic)")

def fix_pdf_generator_naming():
    """Fix PDF generator filename conflicts"""
    
    # Update the _sanitize_filename method in existing pdf_generator.py
    print("⚠️ Manual update needed for src/pdf_generator.py")
    print("💡 Add the enhanced _sanitize_filename method above")

def main():
    print("🔧 Fixing SSL and naming issues...")
    fix_sheets_reader()
    fix_pdf_generator_naming()
    print("\n🎉 Fixes applied!")
    print("\n🚀 Try again with:")
    print('python main.py --spreadsheet-id "12oxYzd7kBiXBAvm_TgLhyfucZUm-e5atvxlJhBV73JY" --output "complete-vietnamese-ebook.pdf" --verbose')
